...
class HrVirementListExportXlsx(models.AbstractModel):
    _name = 'report.th_order_virement.hr_virement_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, virements):
        ...
        headers = [
            ...
            'Mois de paie', 'Année', 'Numéro de chèque', 'Banque', 'Nb fiches'
        ]
        ...
        for row, virement in enumerate(virements, 1):
            ...
            # Type de paiement
            type_paiement = dict(self.env['hr.virement']._fields['type_paiement'].selection).get(virement.type_paiement, '') if hasattr(virement, 'type_paiement') else ''
            ...
            sheet.write(row, 8, len(virement.payslip_ids), data_format)  # Nombre de fiches de paie