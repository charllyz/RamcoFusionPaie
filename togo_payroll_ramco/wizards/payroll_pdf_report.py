...
class PrintPayrollReport(models.TransientModel):
    ...
    _description = "Assistant d'impression du livre de paie mensuel"

    date_debut = fields.Date(string="Date de début du mois", required=True)
    structure_id = fields.Many2one('hr.payroll.structure', string="Structure de paie", required=True)

    def print_report(self):
        data = {}
        selected_structure = self.structure_id
        heads_list = []
        heads = self.env['hr.salary.rule'].search([('struct_id', '=', selected_structure.id), ('active', 'in', (True, False))], order='sequence asc')
        list_rules = []
        for head in heads:
            list_rules = [head.name, head.code]
            heads_list.append(list_rules)

        payslip_ids = self.env['hr.payslip'].search(
            [('date_from', '=', self.date_debut), ('struct_id', '=', self.structure_id.id)])
        list_report = []
        ...
        # Data a envoyer au template d'impression
